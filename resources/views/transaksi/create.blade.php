...
@section('breadcrumb', 'Transaksi / Penjualan / Catat')

@section('content')
<div class="card-erp">
    <div class="card-erp-header">
        <h2 class="card-erp-title">🧾 Form Penjualan</h2>
        ...
        <form method="POST" action="{{ route('transaksi.store') }}" id="formTransaksi">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                <div class="form-group-erp">
                    <label class="form-label-erp">Customer *</label>
                    <select name="m_pelanggan_id" class="form-control-erp @error('m_pelanggan_id') is-invalid @enderror" required>
                        <option value="">-- Pilih Customer --</option>
                        @foreach($pelanggans as $p)
                            <option value="{{ $p->id }}" {{ old('m_pelanggan_id') == $p->id ? 'selected' : '' }}>{{ $p->nama }}</option>
                        @endforeach
                    </select>
                    @error('m_pelanggan_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="form-group-erp">
                    <label class="form-label-erp">Tanggal Transaksi *</label>
            ...
            <h3 style="font-size:15px;color:var(--brand-dark);margin-bottom:16px;">Daftar Item</h3>
                    ...
                    <tr class="item-row">
                        <td>
                            <select name="items[0][m_stok_id]" class="form-control-erp item-select" required>
                                ...
                                @foreach($stoks as $s)
                                    <option value="{{ $s->id }}" data-harga="{{ $s->harga }}" data-stok="{{ $s->jumlah_stok }}">
                                        {{ $s->nama }} (Sisa: {{ $s->jumlah_stok }})
                                    </option>
                                @endforeach
                            </select>
                        </td>
                        <td>
                            <input type="number" name="items[0][harga_satuan]" class="form-control-erp item-harga" value="0" min="0" required>
                        </td>
                        <td>
            ...
            <button type="submit" class="btn-gold-erp" style="width:100%;justify-content:center;font-size:16px;padding:12px;">💾 Simpan Transaksi & Buat Jurnal Otomatis</button>
        </form>
    </div>
</div>
...
<script>
    ...
    const getRowHtml = (index) => `
        <tr class="item-row">
            ...
            <td>
                <input type="number" name="items[${index}][harga_satuan]" class="form-control-erp item-harga" value="0" min="0" required>
            </td>
            <td>
    ...
    // Change select barang -> update harga
    itemsContainer.addEventListener('change', (e) => {
        if (e.target.classList.contains('item-select')) {
            const row = e.target.closest('tr');
            const selectedOpt = e.target.options[e.target.selectedIndex];
            const harga = selectedOpt.dataset.harga || 0;
            row.querySelector('.item-harga').value = harga;
            calculateTotal();
        }
    });

    // Change qty / harga -> update subtotal & grand total
